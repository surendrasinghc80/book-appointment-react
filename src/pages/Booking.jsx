...
const Booking = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post('http://192.168.1.44:8000/api/addappointments', { name: credentials.name, discreption: credentials.description, phoneNumber: credentials.phone, date: credentials.date, time: credentials.time }, { headers: { Authorization: `Bearer ${token}` } })

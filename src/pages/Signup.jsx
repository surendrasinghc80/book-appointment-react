...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post('http://192.168.1.44:8000/api/user/register', { name: credentials.name, email: credentials.email, password: credentials.password, phoneNumber: credentials.phone })
            if (!data.status === 201) {
